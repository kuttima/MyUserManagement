...
/****** Object:  StoredProcedure [DCP].[SP_Add_User]    Script Date: 4/22/2019 3:47:39 PM ******/
SET ANSI_NULLS ON
GO
...
-- Batch submitted through debugger: SQLQuery1.sql|7|0|C:\Users\BBODDU~1\AppData\Local\Temp\~vsC8AF.sql

/*
-- Example of a call

EXEC [DCP].[SP_Add_User] 
	@UserLogin = 'poalaskauser2@tsfd.com',
	@FirstName = 'PO FNAME2',
	@LastName = 'PO LNAME',
	@Password = 'popassword2',
	@Phone = '123456',
	@Role = 'PO',
	@Organization = 'Alaska Native Medical Center'

*/
CREATE
 PROCEDURE [DCP].[SP_Add_User]
...
BEGIN
	...
	DECLARE @Org_Type_Name Varchar(50) = NULL
	...
	SELECT @ROLE_ID = ROLEID FROM DCP.[Role] WHERE [RoleName] = @Role and RecordStatusFlag = 'Active'

	SELECT TOP 1 @ORG_ID = O.OrganizationId,@Org_Type_Name=OT.OrganizationType
	FROM DCP.Organization O		
		LEFT JOIN DCP.OrganizationTypeLookUp OT ON OrganizationTypeKey = OT.OrganizationTypeId 
		WHERE [NAME] = @Organization AND O.RecordStatusFlag = 'Active'

	
	--Debug mode:
	--SELECT @CLO_ID, @PO_ID, @ROLE_ID
	...
	IF((@Role = 'CLO') AND (@ORG_ID IS NOT NULL) and (@Org_Type_Name='CLO' or @Org_Type_Name='CLO/PO'))
	BEGIN
		SET @INPUT_ISVALID = 1
		PRINT 'CLO'
	END
	Else IF((@Role = 'CLO_PO') AND (@ORG_ID IS NOT NULL) and (@Org_Type_Name='CLO/PO'))
	BEGIN
		SET @INPUT_ISVALID = 1
		PRINT 'CLO_PO'
	END
	ELSE IF ((@Role = 'PO') AND (@ORG_ID IS NOT NULL) and (@Org_Type_Name='PO'or @Org_Type_Name='CLO/PO' or @Org_Type_Name='CLO'))
	BEGIN
		SET @INPUT_ISVALID = 1
		PRINT 'PO'
	END 
	ELSE IF(@Role = 'DCP' AND @ORG_ID IS NOT NULL
		AND (@Organization = 'National Cancer Institute')
		)
	BEGIN
		SET @INPUT_ISVALID = 1
		PRINT 'DCP'
	END
	ELSE IF(@Role = 'TRIAdmin' AND @ORG_ID IS NOT NULL
		AND (@Organization = 'Technical Resources International')
		)	
	BEGIN
		SET @INPUT_ISVALID = 1
		PRINT 'TRIAdmin'
	...
	BEGIN
		BEGIN TRAN T	
			...
			VALUES (@FirstName, @LastName, @UserLogin, @Password, GETdATE(), GETdATE(), @Phone, @ORG_ID, 
	...
	BEGIN 
		BEGIN TRAN U	
			...
			UPDATE [DCP].[User] SET [FirstName] = @FirstName , [LastName] = @LastName, [Username] = @UserLogin, [Password] = @Password, [LastLoginDate] = GETDATE(), [PasswordResetDate] = GETDATE(), [Phone] = @Phone, [OrganizationKey] = @ORG_ID
